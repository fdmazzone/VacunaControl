...
By ``consistency'' we mean that the model has a unique solution and produces predictions for the variables that are consistent with their biological meaning.

We consider a population of $N$ individuals with a common birth and death rate $\mu\geq 0$. We assume that this population is divided into $n$ subpopulations (or nodes) according to a given criterion (geographical location, social activity, number of links in a complex network, etc.). Each subpopulation is further divided into susceptible, infected, and immunized individuals, denoted by $S_i$, $I_i$, and $R_i$, respectively. We consider the following network-based SIR epidemic model:
...
\end{empheq}
where $i=1,\ldots,n$ and $N_i=S_i+I_i+R_i$ is the total population of node $i$. The parameters in the model have the following meanings: $\beta_{ij}\geq 0$ are the transmission rates from infected individuals in patch $j$ to susceptible individuals in patch $i$; $u_{i}(t)\geq 0$ (control variable) is the vaccination rate in node $i$ at time $t$; $\gamma^{-1} >0$ is the infectious period; and $\alpha^{-1} >0$ is the immunity period. In Figure \ref{fig:flow_chart} we show a flow chart of the model \eqref{eq:ModeloSIR1}--\eqref{eq:ModeloSIR3}.
...
Then the  system  \eqref{eq:ModeloSIRb1}-\eqref{eq:ModeloSIRb2} is written in compact form $\bold{x}'=\bold{f}(\bold{x},t)$. Given $\bold{x}_0\in\mathbb{R}^{2n}$ we have the corresponding initial value problem
...
One aspect that should be noted and that is often neglected in the literature is the fact that the vector field $\bold{f}$ is not continuous. This is because in optimal control problems it is usually assumed that the control functions $u_i$ are in the space $L^\infty$. Fortunately, we can deal with this type of equations by means of Carathéordory theory  \cite{A.F.Filippov512,EarlA.Coddington236} of differential equations.
 
Following \cite{A.F.Filippov512} we said that  $\bold{x}$ is a \emph{solution} of \eqref{eq:sist_compact} on an  interval $[0,a)$ if and only if $\bold{x}$ is absolutely continuous on each subinterval $[0,c]\subset [0,a)$, it satisfies initial condition and $\bold{x}'=\bold{f}(\bold{x},t)$  for almost everywhere $t\in (0,a)$.   Alternatively, it is equivalent to say that $\bold{x}$ satisfies the integral equation 
...
We note that if $u_i\in L^1([0,+\infty))$ then $\bold{f}$ is a Carathéodory function in the sense of \cite[p. 3]{A.F.Filippov512} on the domain $\Omega\times[0,+\infty)$, where $\Omega$ is an open and bounded set in $\mathbb{R}^{2n}$. That means, $\bold{f}(\bold{x},t)$ is continuous in $\bold{x}$ for almost everywhere $t\in[0,+\infty)$,  $\bold{f}(\bold{x},t)$ is measurable in $t$ for each $\bold{x}\in\Omega$ and there exists $m\in L^1([0,+\infty))$ such that $|\bold{f}(\bold{x},t)|\leq m(t)$ for every $(\bold{x},t)\in \Omega\times[0,+\infty)$.   Consequently we can apply  existence and uniqueness theory for Carathéodory equations \cite[Th. 1, p. 4, Th. 2, p. 5]{A.F.Filippov512}, \cite[Th. 1.1]{EarlA.Coddington236},  we conclude that for any $\bold{x}_0\in \Omega$ there exists a unique local solutions of the  boundary value problem \eqref{eq:sist_compact}  defined in some interval  $[0,\delta)$, $\delta>0$.  
...
The following conjecture states that if $\bold{\beta}$ is an irreducible matrix then the epidemic is transmitted from any node to the rest with infinite speed.
...
Let $\bold{\Psi}=\{\psi_{ij}\}_{i,j=1}^n$ be a real matrix. We say $\bold{\Psi}$ is \textbf{reducible} if there exist a permutation matrix $\bold{P}$ such that $\bold{P}^{T}\bold{\Psi}\bold{P}=\left(\begin{array}{cc}
     \bold{X}  & \bold{Y}\\
     \bold{0} &  \bold{Z}
\end{array}\right)$, where $\bold{X}$ and $\bold{Z}$ are both square matrices. If $\bold{\Psi}$ is not \textbf{reducible} then it is \textbf{irreducible}.
\end{defn}
...
\begin{prop}\label{propo:propagacion.en.la.red} Suppose that $\beta$ is a irreducible matrix. If $S_i(0)\geq 0$, $I_i(0)\geq 0$, $R_i(0)\geq 0$ for every $i\in\{1,2,\ldots,n\}$ and there exists $i_0$ such that $I_{i_0}(0)>0$ then for every $i=1,2,\ldots,n$ and $t>0$ we have that $S_i(t)>0$, $I_i(t)>0$ and $R_i(t)>0$; except in the exceptional case that  $\mu=0$, $\alpha=0$ and  there exists $i'$ such that $S_{i'}(0)=I_{i'}(0)=0$, in which case  
...
In this section we will study the existence of equilibria of model \eqref{eq:ModeloSIR1}-\eqref{eq:ModeloSIR3}  when no control strategies are implemented ($\bold{u}\equiv 0$). 

Let us first recall the approach to the concept of the \emph{basic reproduction number} by means of the \emph{next generation matrix} (NGM). Following \cite{FredBrauer482} next generation matrix for our model  is obtained as follows.  We write $ \bold{f}_{\bold{I}}=\mathscr{F}-\mathscr{V}$, where $\mathscr{F}=\bold{S}\times \bold{\beta}\bold{I}$ and $\mathscr{V}=(\mu+\gamma)\bold{I}$ and calculate the Jacobian matrices of $\mathscr{F}$ and $\mathscr{V}$  at the disease-free equilibrium $\bold{I}=0$ and $\bold{S}=\bold{N}$. We denote by $F$ and $V$ respectively the aforementioned matrices. Then the  NGM is defined by $K=FV^{-1}$.  In the case under study in this section we have that
 ...
 K=FV^{-1}=\begin{pmatrix} \frac{N_1\beta_{11}}{\gamma+\mu} & \frac{N_1\beta_{12}}{\gamma+\mu} & \cdots & \frac{N_1\beta_{1n}}{\gamma+\mu} \\
           ...
           \end{pmatrix}.
\end{empheq}
The $(i, j )$ entry of $K$  is the expected number of secondary infections
in node $i$ produced by an individual initially in node $j$ (see \cite{FredBrauer482}).   The basic reproduction number $\mathscr{R}_0$ is introduced as the spectral radius of the matrix $K$, i.e. $\mathscr{R}_0=\rho(K)$, where $\rho(K)$ denotes  the maximum absolute value of the eigenvalues of $K$.  Observe that if the matrix $\beta$ is irreducible the same is true for the matrix $K$, therefore, from  Perron-Fronenius Theorem, we obtain that $\rho(K)$ is itself an eigenvalue. An important fact to remember is that when  $\mathscr{R}_0<1$ the disease-free equilibrium is locally asymptotically stable.


In the following theorem we show conditions that ensure the occurrence of an endemic equilibrium, i.e. an equilibrium different from the disease-free equilibrium.
 

\begin{thm}
 Suppose that there exits $i\in \{1,2,\ldots,n\}$ such that
...
\end{empheq}
Summing the last two equations we obtain the first one, then it is sufficient to solve the second and third equations. If, in the second equation, we replace $S_i$ by $N_i-I_i-R_i$ and $R_i$ by $\frac{\gamma}{\mu+\alpha}I_i$ we get
...
\end{equation}
which is a system of $n$ equations with $n$ unknowns (we assume $\bold{N}$ given).  We will use the Poincaré-Miranda Theorem, see \cite{PabloAmster218} Theorem 4.4. We define  $F:\mathbb{R}^n\to\mathbb{R}^n$   by means of the expression in the left-hand side of \eqref{eq:F(I)}. We will look for a zero of $F$ in the set  $\Omega=[0,N_1]\times \cdots\times [\epsilon,N_i]\times \cdots \times [0,N_n]$ for $\epsilon>0$ which we will specify later. Since $0\notin \Omega$, the zero of $F$ that we will find will be different from the disease-free equilibrium. 

For simplicity, we denote  by $F_i(I_j=a)$ the result of replacing the  $I_j$ component by $a$ in the expression of $F_i$ while the other variables remain arbitrary. In order to see that the hypotheses of the Poincaré-Miranda theorem are satisfied, let us observe that a simple calculation shows that  $F_k(I_k=0)\leq 0 \leq F_k(I_k=N_k)$, $k=1,\ldots,n$. This fact is sufficient to apply the above theorem, but nothing prevents us from finding the disease-free equilibrium again. To overcome this problem, we make the following calculation
\begin{empheq}{multline*}
 ...
 \leq A\beta_{ii}\varepsilon^2+ (\gamma+\mu)\left[A \sum_{j\neq i}^n K_{ij}\frac{N_j}{N_i}-\beta_{ii}+1    \right]\varepsilon
 \end{empheq}
 Since the term in square brackets is negative, $ F_i(I_i=\varepsilon)$ will be negative for sufficiently small $\varepsilon$ and for all $I_j$ with $ j \neq i$. 
...
\begin{comentario} It is well known that the occurrence of endemic equilibria is related to the fact that $\mathscr{R}_0>1$ (see \cite[Ch. 3]{FredBrauer482}). We want to show that the condition \eqref{eq:condicion.equilibrio} ensures , in a sense, a similar statement.   If condition \eqref{eq:condicion.equilibrio} were true for all $i=1,\ldots,n$ , then the statement  (M${}_35$) of Theorem 2.3 in \cite{AbrahamBerman}  would be satisfied with $d_j=N_j$ and for the matrix $K-I_n$, where $I_n$ denotes the identity matrix of order $n$ (observe that condition \eqref{eq:condicion.equilibrio}  implies that  $K-I_n$ has  all positive diagonal elements).   Therefore,  Theorem 2.3 in \cite{AbrahamBerman} implies that every real eingevalue of $K-I$ is positive. In particular $\rho(K-I)=\rho(K)-1>0$. We conclude that $\mathscr{R}_0>1$.
...
The goal  is to show that, in some sense, the number of vaccinated individuals at time $t$, $u_i(t)S_i(t)$, approaches to zero as $t\to+\infty$  for any $i$. This regardless of the vaccination campaign. For example, a very aggressive campaign, so that the vaccination rate $u_i$ are high, will imply that the number of susceptible individuals converges to zero fast enough so that the total number of vaccinated $u_iS_i$ also tends to zero.
...
that $f_\gamma$ are continuous and  $f_\gamma\to f$  almost everywhere and in $L^p$-norm with $1\leq p<\infty$.  
...
The following proposition to expresses the fact that  the  epidemic is extinguished when $t\to\infty$ and, in a certain sense, the total quantity of vaccines applied $ u_iS_i$ goes to zero when $t\to\infty$. 

We recall that $\mu=\alpha=0$ and that  the infection period is given by the quantity $\gamma^{-1}$.   

\begin{prop} We consider the function $\varphi$ defined by  $\varphi(t)=e^{-t}$, when $t>0$ and $\varphi(t)=0$ otherwise. The regularized function $(u_iS_i)_\gamma$  of the total number of vaccines applied at $i$th node  per unit of time goes to zero when $t\to\infty$. More concretely
\[
...
In this section, we will study the problem of the optimal distribution of vaccines subject to a limited supply of them.  More precisely, given $T>0$, we consider the following Lagrange optimal control problem with mixed inequalities constraints:
