...
Before developing the main results of this section, we introduce a more compact notation. The expression $\bold{a}\cdot\bold{b}$ will represent the usual dot product between the $n$-dimensional vectors $\bold{a}$ and $\bold{b}$, and we will write $\bold{a}>0$ (or $\bold{a}\geq 0$) to denote a vector with all positive (non-negative) components.

In this section, we will study the problem of the optimal distribution of vaccines subject to a limited supply. By an optimal campaign we mean one that minimizes the total number of infected individuals across all nodes accumulated over the time interval $[0,T]$. More precisely, given $T>0$, we consider the following Lagrange optimal control problem with mixed inequality constraints:
...
\noindent\textrm{a)} State  equations
...
\end{align}
where $\bold{S}_0$ and $\bold{I}_0$ are known vectors.
...
The objective function in \eqref{OP:minim} represents $\gamma$ times the total number of infections over the interval $[0,T]$. Therefore, minimizing the objective function is equivalent to minimizing the latter quantity.

The function $M(t)$, which is assumed to be continuous and strictly positive, represents the vaccine supply rate for the total population at time $t$ per unit of time. Hence, constraint \eqref{res:cant.de.vacunados} means that the total number of vaccines distributed throughout the network per unit of time does not exceed $M(t)$.



\section{Existence minimizers}

In this section, we prove that the problem \eqref{OP:minim}--\eqref{res:u>=0} satisfies the hypotheses of the \emph{Filippov--Cesari Theorem} (see \cite[Th. 9.3.i]{L.Cesari368}) and, consequently, admits a solution.

In what follows, we use the notation introduced in \eqref{def:x} and \eqref{def:func.fS.fY}. Additionally, we consider the Lagrangian function
...
\begin{proof}
First, we observe that since $0\leq S_i\leq N_i$, $0\leq I_i\leq N_i$, and $t\in[0,T]$, the domain of the time-state variables can be chosen as a compact set, which we denote by $A$. This verifies one of the hypotheses of Theorem 9.3.i in \cite{L.Cesari368}.

Additionally, we need to prove that the set
$$ \mathcal{M}:=\left\{(t,\bold{x},\bold{u}): (t,\bold{x})\in A\ \text{and}\ \bold{u}\in \mathcal{U}(t,\bold{S})  \right\}  $$ 
is a compact set in the time-state-control variable space. This is true since, from \eqref{OP:cond.inic} and \eqref{eq:eq_integralS}, we have that $S_i(t)\geq \delta>0$ for every $t\in[0,T]$. Therefore, $\mathcal{M}$ is a closed and bounded set, and hence compact.

Finally, we need to establish that the set $$ \mathcal{Q}(t,\bold{x}):=\left\{(z_0,\bold{z})\in \mathbb{R}^{1+2n}:z_0\geq f_0(t,\bold{x},\bold{u}), \bold{z}=f(t,\bold{x},\bold{u}),\ \text{and}\ \bold{u}\in \mathcal{U}(t,\bold{S}) \right\} $$
is a convex set, and this is an immediate consequence of the fact that $f$ is a linear function with respect to the control variable $\bold{u}$.
\end{proof}

\section{Necessary conditions}\label{sec:nece}

In this section, we apply the Pontryagin Maximum Principle \cite[Th. 4.1]{A.Seierstad499}, \cite{hartl1995survey} to obtain necessary conditions for $(\bold{x}^*,\bold{u}^*)=(\bold{S}^*,\bold{I}^*,\bold{u}^*)$ to be a solution of the optimal problem \eqref{OP:minim}--\eqref{res:u>=0}.
...
$$\bold{p}_\bold{S}(t)=(p_{S1}(t),\ldots,p_{Sn}(t)) \ \text{and}\ \bold{p}_\bold{I}(t)=(p_{I1}(t),\ldots,p_{In}(t))$$
and the Hamiltonian function defined by
\begin{empheq}{multline}\label{eq:hamiltoniano}
\mathscr{H}(\bold{S},\bold{I},\bold{u},\bold{p}_\bold{S},\bold{p}_\bold{I})=
-p_0 \sum_{i=1}^{n} I_{i}  
 +\bold{p}_{\bold{S}}\cdot \bold{f}_{\bold{S}}(\bold{S},\bold{I},\bold{u})+ \bold{p}_\bold{I}\cdot \bold{f}_{\bold{I}}(\bold{S},\bold{I},\bold{u}),
\end{empheq} 
...
We suppose that $(\bold{S}^*,\bold{I}^{*},  \bold{u}^{*})$ is a  solution of \eqref{eq:optimal_model}. From now on any function evaluated at  $(\bold{S}^*,\bold{I}^{*},  \bold{u}^{*})$ will be indicated by a superscript $\ast$.

We combine the constraints in a vector valued function 

$$\bold{h}(\bold{S},\bold{u},t)=(-\bold{u}\cdot \bold{S}+M(t),u_1,\ldots, u_n).$$
Then, the constraints can be expressed compactly as $\bold{h}\geq \bold{0}$, where the order relation between vectors is understood componentwise. According to \cite[Th. 4.1]{A.Seierstad499} (see also \cite{hartl1995survey}), in order to apply the Maximum Principle, the constraint qualification condition must be satisfied. This condition means that the gradients of the active constraints (that is, those with indices $i$ such that $h_i=0$) are linearly independent. In our case, the following condition must be satisfied
...
n+1=
 \operatorname{rank}
                    ...
                    \right]
\end{equation}
We note that if $-\bold{u}\cdot \bold{S}+M(t)\neq 0$, then the qualification condition follows immediately. In the case where $-\bold{u}\cdot \bold{S}+M(t)=0$ (the last constraint is active), we can prove that the matrix on the right-hand side of \eqref{eq:cons_qua} can be row-reduced to
\begin{equation*}\label{eq:cons_qua1}
                    \left[
                    \begin{array}{c c c|c c c c }
                      1      & \cdots & 0      & u_1    & \cdots& 0 &  0       \\
                      \vdots & \ddots & \vdots & \vdots & \ddots&\vdots & \vdots   \\
...
We are now in a position to apply the Maximum Principle. First, we observe that, by virtue of equations (29), (34), and (35c) in \cite[Th. 4.1]{A.Seierstad499}, we may assume that $p_0=1$ in \eqref{eq:hamiltoniano}. On the other hand, we infer that there exist functions $\bold{p}^\ast_{\bold{S}},\bold{p}^\ast_{\bold{I}}:[0,T]\to\mathbb{R}^n$ and multipliers $\bold{q}:=(q_0(t),\bar{\bold{q}}(t)):=(q_0(t),\ldots,q_n(t))\in\mathbb{R}^{n+1}$ such that the following conditions are satisfied.
...
\mathscr{H}(\bold{S}^\ast,\bold{I}^\ast,\bold{u}^\ast,\bold{p}_\bold{S}^\ast,\bold{p}_\bold{I}^\ast)\geq 
\mathscr{H}(\bold{S}^\ast,\bold{I}^\ast,\bold{u},\bold{p}_\bold{S}^\ast,\bold{p}_\bold{I}^\ast)
\end{empheq}

From the fact that $\bold{h}(\bold{S},\bold{u},t)\geq 0$ and from \eqref{eq:rest-act} we have that if $u_j^\ast(t)\neq 0$ then $q_j(t)=0$. Thus, from \eqref{eq:L_critico}, $q_0S_j^*=-p_{Sj}S_j^*$ when $u_j\neq 0$. Therefore, since $S_j^*\neq  0$ (see Proposition \ref{propo:propagacion.en.la.red}), we can replace equation \eqref{eq:cons_adjS} by 
...
\begin{prop}\label{prop:cond_nec} We suppose that $(\bold{S}^*,\bold{I}^{*},  \bold{u}^{*})$ is a  solution of \eqref{OP:minim}--\eqref{res:u>=0}.  Let $F(t)\subset \{1,\ldots,n\}$  be the subset of indices defined by 
$$j\in F(t) \Longleftrightarrow p_{S^\ast_j}(t) = p_{\text{min}}(t):=
\min\{p_{S^\ast_1}(t),\ldots, p_{S^\ast_n}(t)\}.$$
Then 
...
\end{equation}
Therefore  \eqref{eq:maximalidad} implies that for each $t\in[0,T]$ and for every $\bold{u}$ with  $\bold{u}\geq \bold{0}$ and $\bold{u}\cdot\bold{S}^\ast\leq M(t)$ we have
...
\[
 \bold{p}_{\bold{S}}^\ast\cdot (\bold{S}^\ast\times \bold{u}^\ast)\leq p_{\text{min}}(t) M\leq p_{\text{min}}(t) \bold{S}^*\cdot \bold{u}^*.
\]
Hence
\[
...
\begin{enumerate}
\item Find $\bar{u}$.
\item Solve the system \eqref{eq:ModeloSIRb1}--\eqref{eq:ModeloSIRb2}, augmented with the equation $J'=\gamma\sum I_j$, using $\bar{u}$ instead of $u$.
\item Return $J(T)$.
\end{enumerate}
...
A difficulty arises when applying numerical methods to solve the system \eqref{eq:OP_2}--\eqref{eq:OP_3}. When, for some $i$, $S_i$ approaches zero, it is possible that the corresponding immunization rate $u_i$ tends to infinity. As a result,  equations \eqref{eq:OP_2}--\eqref{eq:OP_3} exhibit characteristics of a stiff system, and conventional integrators may fail to solve them accurately. To address this issue, we impose an upper bound on the rates $u_i$ (specifically, $u_i\leq 5\gamma$), and we select a numerical method suitable for stiff problems. In particular, we use a fifth-order Radau IIA method (see \cite{ErnstHairer1546}).



Minimization algorithms require an initial guess, and starting from a good candidate is important. In our experiments, we used a uniform vaccination policy as the initial guess; that is, vaccines were distributed evenly so that each node was vaccinated at the same rate.
...
\subsection{Two-node experiment}

We use the parameters listed in Table \ref{tabla:2_nodos} and assume that $I_1(0)=0.1N_1$ and $I_2(0)=0$. We tested four constraint-compatible minimization algorithms implemented for the \texttt{minimize} function of the \texttt{scipy.optimize} module. These are Constrained Optimization by Linear Approximations (COBYLA), see \cite{powell1994direct}; Constrained Optimization by Quadratic Approximations (COBYQA), see \cite{rago_thesis,razh_cobyqa}; Sequential Least Squares Quadratic Programming (SLSQP), see \cite{10.1145/192115.192124,kraft88}; and \texttt{trust-constr}, which is a trust-region algorithm for constrained optimization (see \cite{A.R.Conn1530}). The execution times for each of these algorithms are shown in Table \ref{tabla:tiempos}.
...
\begin{table}[h]
 \begin{center}
        ...
        \end{tabular}
\end{center}\caption{Execution times (minutes) for the two-node experiment} \label{tabla:tiempos}
...
\end{figure}
The results are displayed graphically in Figures \ref{fig:resul2x1}--\ref{fig:vaccination_rates}. In Figure \ref{fig:resul2x1}, we plot the accumulated number of infected individuals, $J$, versus $t$. All numerical optimization algorithms show comparable performance, improving upon the uniform vaccination policy. It can also be observed (see Figure \ref{fig:restric2x1}) that all algorithms satisfy the constraints to a similar order of accuracy. Figures \ref{fig:adjoint} and \ref{fig:vaccination_rates} are consistent with Proposition \ref{prop:cond_nec}; that is, all available vaccine stock at time $t$ is administered at node $i$, where $p_{S_i}$ attains the minimum of the vector $\mathbf{p_S}$.




\subsection{The $5\times 5$ experiment}

To test the capabilities of our algorithm, we addressed a larger-scale problem by considering networks with a large number of nodes. This leads to minimization problems with many degrees of freedom. Even for modest values of $k$ and $n$ ($k$ being the number of interpolation points and $n$ the number of nodes), the space over which the objective function must be minimized becomes very high-dimensional and challenging.

We consider a metapopulation network of $25$ nodes (identified with the set $D=\{(r,s)\mid r,s=1,\ldots,5\}$), arranged in a $5\times 5$ grid graph. In our model, individuals at node $(r,s)$ interact only with individuals at their nearest neighbors in the four cardinal directions (N, S, E, W). Naturally, nodes at the edges of the network have fewer neighbors. Nodes are indexed by $i=1,\ldots,25$, assigned from top to bottom and from left to right.

If $i$ and $j$ are neighbors, we assume that $\beta_{ij}=0.08$. We propose the following formula for the intranode infection rates:
\[
\beta_{ii}=1.1\left(\frac{2}{(r-5)^2+(s-5)^2+1}+1\right),
\]
where $i$ corresponds to node $(r,s)$. Note that the intranode infection rates range from $1.16$ to $3.3$, with the maximum value attained at the bottom-right node.
We assume that $N_i=1$ for every $i=1,\ldots,25$, $\gamma=1$, $\mu=0.01$, and $\alpha=0.2$. Finally, we assume that initially only node $i=1$ contains infected individuals, with a total number equal to $0.1$. The function $M(t)$ is assumed to be constant and equal to $5\%$ of the total population, that is,
\[
M(t)=0.05\sum_{i=1}^{25}N_i=1.25.
\]
...
\]
The parameter $\epsilon$ in the formula is introduced to prevent infinite vaccination rates. In the experiments, it was set to $0.001$.

We present the results obtained with the \texttt{trust-constr} algorithm, since it was the only one that produced a perceptible improvement over the initial candidate. In Figure \ref{fig:5x5_J}, we plot the cumulative number of infected individuals corresponding to the two vaccination schemes: the uniform policy and the solution obtained with the \texttt{trust-constr} algorithm. We observe that the latter produces $8\%$ fewer infections over the interval $[0,T]$, relative to the total population.

In Figure \ref{fig:5x5_restri}, we represent the total number of vaccinated individuals at time $t$, $\sum_{i=1}^{n}u_i(t)S_i(t)$, and observe that the result satisfies the constraint satisfactorily.

To study whether the conclusion of Proposition \ref{prop:cond_nec} is satisfied, we define $i_t=\arg\min(\bold{p_S}(t))$, sort the vector $\bold{u}(t)$ from largest to smallest, and determine the position $j(t)$ occupied by $i_t$ in that ordered vector. According to Proposition 5.1, the value of $j(t)$ should be $1$. However, this does not occur, which indicates that the result produced by the algorithm is not a global minimum, but is presumably close to a local minimum. Nevertheless, we can observe a tendency for $j(t)$ to take small values. In Figure \ref{fig:5x5_chequeo}, we represent $j(t)$ and, with the orange line, its average value, which is equal to $6.24$.

Figures \ref{fig:figura_con_subfiguras} and \ref{fig:figura_con_subfiguras2} present density diagrams for the variables $\bold{I}$ and $\bold{u}\cdot\bold{S}$ at various time steps $t$.
...
\begin{figure}[htbp]
    ...
    \caption{Evolution of the infected population under an optimal vaccination schedule.}
...
\begin{figure}[htbp]
    ...
    \caption{Distribution of vaccines in the network under an optimally controlled vaccination schedule.}
    \label{fig:figura_con_subfiguras2}
\end{figure}

\section{Conclusions}

We studied a metapopulation SIR model with vaccination that incorporates demographic effects and a disease that confers only temporary immunity. We considered variable vaccination rates given by the vector $\bold{u}(t)=(u_1(t),u_2(t),\ldots,u_n(t))$, where $u_i(t)$ denotes the vaccination rate in patch $i$. We identified a property of the transmission matrix that guarantees the spread of the disease to all patches of the metapopulation. We also showed that, when $\bold{u}(t)=\bold{0}$ and under suitable conditions, an endemic equilibrium exists.

We then addressed an optimal control problem in which the total number of individuals vaccinated per unit of time, namely $\sum_{i=1}^{n}u_i(t)S_i(t)$, cannot exceed a known function $M(t)$, representing the vaccine supply rate at time $t$. Under this constraint, we looked for the function $\bold{u}(t)$ that minimizes the total number of infected individuals over the interval $[0,T]$. We proved that this problem admits a solution and, using Pontryagin's Maximum Principle, showed that the optimal control has a bang-bang structure. We also designed and implemented a numerical method, using Python libraries, first for a metapopulation with two patches and then for a metapopulation whose transmission matrix is based on a lattice graph. The numerical results are consistent with the theoretical findings.
